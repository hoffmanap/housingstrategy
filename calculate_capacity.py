...
def process_district_capacity(district_num, user_levers, land_use_map):
    ...
    if os.path.exists(footprint_file):
        ...
        if not footprints.empty:
            ...
            if pid_col and pid_col in footprints.columns:
                ...
                parcels = parcels.drop(columns=['total_footprint_area', 'total_interior_area'], errors='ignore').merge(footprint_agg, on=pid_col, how='left')
                ...
                if not footprints_joined.empty:
                    ...
                    parcels = parcels.drop(columns=['total_footprint_area', 'total_interior_area'], errors='ignore').join(footprint_agg, how='left')
                    parcels = parcels.rename(columns={'calc_footprint_area': 'total_footprint_area', 'calc_interior_area': 'total_interior_area'})
    ...
    for idx, parcel in parcels.iterrows():
        ...
        # ELIMINATE PARKING LEVER IMPLEMENTATION
        ...
        if is_residential:
            ...
            # FIXED SYNTAX ERROR: 2000 instead of 2,000
            ...
            if not policy_applied and user_levers.get('allow_middle_housing', False):
                ...
                if tier == '16-plex' and lot_area >= 6000:
                    gross_sim_units = 16
                elif tier == '8-plex' and lot_area >= 5000:
                    gross_sim_units = 8
                else:
                    gross_sim_units = 4 
                policy_applied = True
...
def run_master_simulation(user_levers):
    # This block exists strictly for terminal/local testing. 
    # The browser UI bypasses this and executes directly to pull JSON data.
    pass
